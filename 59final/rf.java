Triangle one, two, tree, four;
Lobster lob;
Boat[] sail = new Boat[3];
int many=5;
...
void setup() {
  ...
  reset();
  for(int i = 0; i < sail.length; i++){
   sail[i] = new Boat();
  }
  one = new Triangle(0,0);
  two = new Triangle (0,100);
  tree = new Triangle(0, 200);
  four = new Triangle(0, 300);
  lob = new Lobster();
...
class Triangle{
  ...
  Triangle(float tmpX, float tmpY ){
  ...
  void tshow(){
    fill(250,18,18);
    triangle(x-50+width,surface+20+y,x+width,surface+10+y,x+width,surface+50+y);
  }
